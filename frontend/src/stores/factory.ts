...
export class EntityStore {
  ...
  async updateRecord(path: string, id: number, input: Partial<DomainRecord>) { this.patch({ loading: true }); try { await request<DomainRecord>(`/${path}/${id}`, { method: 'PUT', body: JSON.stringify(input) }); await this.load(path); } catch (error) { this.patch({ loading: false, error: error instanceof Error ? error.message : String(error) }); throw error; } }
