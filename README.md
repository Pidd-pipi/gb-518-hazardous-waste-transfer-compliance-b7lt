...
- 产废单位档案可维护可选的备案去向（处理厂名称 + 经营许可证号，可备案多处，支持撤销），新建/修改单位时在同一事务内更新并在档案列表展示。
- 联单提交和发运前会重新核验产废许可为 `active`、承运资质为 `verified`，且双方证照仍在有效期内；已备案去向的单位，联单去向必须与当前备案逐项对上，缺少或已撤销时返回具体地点和许可证问题，联单与版本保持不变。
- 单位后续调整备案不回溯影响已提交和运输中的联单（在途联单照常签收），只有草稿重新提交会被拦下；未备案任何去向的单位沿用手填去向旧流程。
